Shader "URPCustom/PBR" {
    Properties {
        ...
        _SSRIntensity ("_SSRIntensity", Range(0, 1)) = 0.6
    }
    ...
    SubShader {
        ...
        TEXTURE2D(_ssrResult);
        SAMPLER(sampler_ssrResult);
        ...
        Pass {
            ...
            half4 FRAG(v2f i) : SV_TARGET {
                ...
                float3 DirectColor =(DirectSpeColor + DirectDiffColor) * mainLight.shadowAttenuation;
                ...
                //间接光部分计算完成
                half3 Color = IndirColor + DirectColor;

                float2 uv = i.positionCS.xy / _ScaledScreenParams.xy;
                half3 ssr = SAMPLE_TEXTURE2D(_ssrResult, sampler_ssrResult, uv).rgb * _SSRIntensity;
                //貌似是第一次的ssrResult是未初始化的，不加上这行会有黑色块
                ssr = max(ssr, 0);
                Color += ssr;   
